...
MODEL_TYPE = "tiny"  # "small"  # use larger models for better performance
...
def execute_blocking_whisper_prediction(
    ...
    segments, info = model.transcribe(
        ...
        beam_size=5,
        vad_filter=VAD_FILTER,  # transcribe only when vad is triggered
...
async def predict(
    ...
    try:
        ...
        result = await asyncio.get_running_loop().run_in_executor(
            None,
            execute_blocking_whisper_prediction,
            model,
            audio_data_array,
            language_code,
        )
