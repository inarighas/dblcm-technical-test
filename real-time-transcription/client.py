...
LENGHT_IN_SEC: int = (
    6  # We'll process this amount of audio data together maximum
)
NB_CHANNELS = 1
RATE = 16000
CHUNK = RATE
...
def send_audio_to_server(audio_data) -> str:
    ...
    return result["prediction"], result["language"]
...
def producer_thread():
    ...
    while True:
        ...
        audio_queue.put(
            audio_data
        )  # Put the 5-second audio data into the queue
...
def consumer_thread(stats):
    while True:
        ...
        try:
            transcription, language = send_audio_to_server(
        ...
        transcription_to_visualize = transcription.ljust(
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"The average latency is {np.mean(stats['overall'])+STEP_IN_SEC:.4f}s"
        )
